...
// QSYS Autonomous Trading Engine — Paper/Live modes
...
function generateSignal(closes: number[], highs: number[], lows: number[]): Signal {
  ...
  // Signal 1: RSI extremes (mean reversion)
  ...
  // Signal 3: Bollinger Bands (volatility breakout / mean reversion)
  ...
  // Signal 4: Price vs EMA alignment
  ...
  return {
    direction,
    strength,
    reasoning: reasons.join(" | ") || "No signal",
    rsi,
    emaFast: emaFastNow,
    emaSlow: emaSlowNow,
    atr,
...
async function igPlaceOrder(session: IGSession, epic: string, direction: string, size: number, stopLevel: number, limitLevel: number): Promise<any> {
  ...
  try {
    ...
    return {
      status: conf.dealStatus ?? "UNKNOWN",
      dealId: conf.dealId ?? "",
      level: conf.level ?? 0,
      reason: conf.reason ?? "",
    };
        ...
        if (!instruments || instruments.length === 0) {
          ...
          for (const inst of toScan) {
            ...
            await sb.from("trading_signals").insert({
              ...
              price_at_signal: priceData.current,
              rsi_value: sig.rsi,
              ema_fast: sig.emaFast,
              ema_slow: sig.emaSlow,
              atr_value: sig.atr,
            ...
            // Check paper trades to close
            for (const pt of (openPaperTrades ?? [])) {
              if (pt.epic === inst.epic) {
                const hitStop = pt.direction === "BUY"
                  ? priceData.current <= pt.stop_loss
                  : priceData.current >= pt.stop_loss;
                const hitTP = pt.direction === "BUY"
                  ? priceData.current >= pt.take_profit
                  : priceData.current <= pt.take_profit;
                ...
                if (hitStop || hitTP) {
                  ...
                  await sb.from("trading_paper_trades").update({
                    status: hitStop ? "STOPPED" : "TP_HIT",
                    exit_price: priceData.current,
                    pnl,
          ...
          if (canTrade) {
            for (const s of signals) {
              ...
              if (mode === "paper") {
                ...
                await sb.from("trading_signals").update({ acted_on: true })
                  .eq("instrument_id", s.instrument.id)
                  .order("created_at", { ascending: false })
                  .limit(1);
              } else if (mode === "live") {
                const orderResult = await igPlaceOrder(
                  session, s.instrument.epic, s.signal.direction, stake,
                  Math.round(stopLoss * 10) / 10,
                  Math.round(takeProfit * 10) / 10,
                );
          ...
          await sb.from("trading_equity_log").insert({
            equity: equity + totalPnl,
            cash: available,
            running_pnl: totalPnl,
            n_positions: currentPositions.length,
            drawdown_pct: totalPnl < 0 ? Math.abs(totalPnl) / equity : 0,
            mode,
          });
          ...
          result = {
            ...
            openPaperTrades: openPaperCount,
            timestamp: new Date().toISOString(),
          };
        }

      } else if (action === "positions") {
        // ── POSITIONS ENDPOINT (for dashboard) ──
