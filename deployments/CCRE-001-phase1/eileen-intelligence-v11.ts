// eileen-intelligence/index.ts — v11 (CCRE-001 Path B refactor + KLIA-001 §7 surveillance integration)
...
// --- SURVEILLANCE INTENT DETECTION (KLIA-001 §7) ---
function detectSurveillanceIntent(message: string): boolean {
  const lowerMsg = message.toLowerCase();
  const surveillanceTerms = [
    'parliament', 'parliamentary', 'hansard', 'debate', 'debated',
    'bill ', 'bills ', ' bill', 'reading', 'royal assent', 'committee stage',
    'legislation news', 'legislative', 'regulatory change', 'regulatory update',
    "what's new", 'whats new', 'latest development', 'latest news',
    'recent change', 'recent update', 'coming into force', 'commencement',
    'new law', 'new legislation', 'employment law change', 'law reform',
    'horizon', 'pipeline', 'surveillance', 'monitoring',
    'acas guidance', 'ehrc', 'hse guidance', 'ico guidance',
    'statutory instrument', 'si 20', 'regulation change',
    'era 2025', 'employment rights act 2025', 'fair work agency',
    'what is changing', 'what has changed', 'what changed',
    'any updates', 'any news', 'keep me updated', 'tracking',
    'government', 'minister', 'secretary of state',
    'consultation', 'green paper', 'white paper'
  ];
  return surveillanceTerms.some(term => lowerMsg.includes(term));
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // --- SURVEILLANCE INTELLIGENCE LAYER (KLIA-001 §7) ---
    let surveillanceSummary: any = null;
    try {
      const { data: summaryData, error: summaryError } = await supabase.rpc('get_surveillance_summary');
      if (!summaryError && summaryData) {
        surveillanceSummary = summaryData;
      }
    } catch (e) {
      console.log('Surveillance summary fetch failed (non-blocking):', (e as Error).message);
    }

    let surveillanceDetail: any = null;
    const isSurveillanceQuery = detectSurveillanceIntent(message);
    if (isSurveillanceQuery) {
      try {
        const { data: detailData, error: detailError } = await supabase.rpc('get_surveillance_detail');
        if (!detailError && detailData) {
          surveillanceDetail = detailData;
        }
      } catch (e) {
        console.log('Surveillance detail fetch failed (non-blocking):', (e as Error).message);
      }
    }

    // Build surveillance context string
    let surveillanceContext = '';
    if (surveillanceSummary) {
      surveillanceContext += '\n\n--- SURVEILLANCE SUMMARY (live platform data) ---\n';
      surveillanceContext += 'Tracked employment bills: ' + surveillanceSummary.tracked_bills_total + '\n';
      surveillanceContext += 'Published (curated) entries: ' + surveillanceSummary.tracked_bills_published + '\n';
      surveillanceContext += 'Latest news item: ' + (surveillanceSummary.latest_news_date || 'none') + ' — ' + (surveillanceSummary.latest_news_title || 'n/a') + '\n';
      surveillanceContext += 'Pending surveillance alerts: ' + surveillanceSummary.active_alerts_count + '\n';
      surveillanceContext += 'Automated pipelines running (last 48h): ' + surveillanceSummary.pipelines_running + '\n';
      surveillanceContext += 'Data freshness: ' + (surveillanceSummary.data_freshness_hours ? Math.round(surveillanceSummary.data_freshness_hours) + ' hours ago' : 'unknown') + '\n';
      surveillanceContext += '--- END SURVEILLANCE SUMMARY ---\n';
    }
    if (surveillanceDetail) {
      surveillanceContext += '\n--- SURVEILLANCE DETAIL (full intelligence context) ---\n';
      if (surveillanceDetail.recent_news && surveillanceDetail.recent_news.length > 0) {
        surveillanceContext += '\nRECENT PARLIAMENTARY & REGULATORY NEWS (last 30 days):\n';
        for (const item of surveillanceDetail.recent_news) {
          surveillanceContext += '- [' + item.published_date + '] ' + item.title;
          if (item.summary) surveillanceContext += ' — ' + item.summary;
          if (item.url) surveillanceContext += ' (' + item.url + ')';
          surveillanceContext += '\n';
        }
      }
      if (surveillanceDetail.tracked_legislation && surveillanceDetail.tracked_legislation.length > 0) {
        surveillanceContext += '\nTRACKED EMPLOYMENT LEGISLATION (auto-monitored daily):\n';
        for (const item of surveillanceDetail.tracked_legislation) {
          surveillanceContext += '- ' + item.title + ' [' + item.type + '] — Stage: ' + item.stage + ', Status: ' + item.status;
          if (item.is_published && item.headline) {
            surveillanceContext += ' | ' + item.headline;
          } else if (!item.is_published) {
            surveillanceContext += ' | [Impact analysis pending CEO review]';
          }
          if (item.last_checked) surveillanceContext += ' | Last checked: ' + item.last_checked;
          surveillanceContext += '\n';
        }
      }
      if (surveillanceDetail.recent_alerts && surveillanceDetail.recent_alerts.length > 0) {
        surveillanceContext += '\nRECENT SURVEILLANCE ALERTS:\n';
        for (const item of surveillanceDetail.recent_alerts) {
          surveillanceContext += '- [Class ' + item.alert_class + '] ' + item.title + ' — ' + item.alert_type + ' — Detected: ' + item.detected_at;
          if (item.summary) surveillanceContext += ' — ' + item.summary;
          surveillanceContext += '\n';
        }
      }
      if (surveillanceDetail.pipeline_health && surveillanceDetail.pipeline_health.length > 0) {
        surveillanceContext += '\nPIPELINE STATUS (internal — do not surface to user):\n';
        for (const item of surveillanceDetail.pipeline_health) {
          surveillanceContext += '- ' + item.pipeline_code + ': ' + item.status + ' (last run: ' + item.last_run + ', found: ' + item.records_found + ', new: ' + item.records_new + ')';
          if (item.error) surveillanceContext += ' [ERROR: ' + item.error + ']';
          surveillanceContext += '\n';
        }
      }
      surveillanceContext += '--- END SURVEILLANCE DETAIL ---\n';
    }
    ...
    const fullSystemPrompt = SYSTEM_PROMPT_CONSTITUTIONAL + dateContext + ragContext + temporalContext + limitsContext + surveillanceContext;
